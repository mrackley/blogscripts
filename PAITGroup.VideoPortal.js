/*
 * PAITGroup.VideoPortal- Create digital signatures in SharePoint
 * Version 1.0 
 ...
 */
/**
 * @description Integrates O365 Video Portal with ShareSharePoint Online site
 * @type jQuery
 * @name PAITGroup.VideoPortal
 * @category Plugins/PAITGroup
 * @author Mark Rackley / http://www.markrackley.net / http://www.paitgroup.com / mrackley@paitgroup.com
 */

	document.write(
	'<div id="MediaPortal" class="MediaPortal">'+
	'<div class="MediaPlayer" ><span class="ms-font-xl ms-fontColor-white">PAIT Group Video Portal Player</span><i class="ms-Icon ms-font-xl ms-Icon--xCircle ms-fontColor-white arrow" aria-hidden="true"  onclick="$(\'#MediaPortal\').fadeOut();$(\'#video\').empty();"></i><div id="video"></div></div>'+
	'</div>'+
	
	'<span id="channels" class="ms-font-l ms-fontColor-blue"></span>'+
	'<br><br>'+
	'<div id="results" >'+
	'<span class="ms-font-xl ms-fontColor-blue">Filter: <input type="text" id="filterBox" onkeyup="FilterVideos(this.value);"></span>'+
	'<div id="PAITApps"></div>'+
	'</div>');
	...
	function SelectChannel(value)
	{
		...
		call.done(function (data,textStatus, jqXHR){
			...
			for (index in data.d.results)
			{
				var tile = "<div class='PAITApp-Tile' onclick='AddVideo(\""+data.d.results[index].ID+"\")'>" +
				"  <div class='front'> " +
...
//				"  <div class='back'>" +
				...
				"		</div>"+
				"		<div class='ms-Grid-row'>"+
				"			<span class='ms-Grid-col ms-u-sm4 ms-font-m-plus ms-fontColor-white'>File name:</span>"+
				"			<span class='ms-Grid-col ms-u-sm8 ms-font-m-plus ms-fontColor-white'>"+data.d.results[index].FileName+"</span>"+
				"		</div>"+
				"		<div class='ms-Grid-row'>"+
...
//				"  </div> " +
	...
	{
		var src = GVideoPortalURL +"/_layouts/15/VideoEmbedHost.aspx?chId=" + GVideoChannel + "&amp;vId=" + id + "&amp;width=640&amp;height=360&amp;autoPlay=true&amp;showInfo=true";
